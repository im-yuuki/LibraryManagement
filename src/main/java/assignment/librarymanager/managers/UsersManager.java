...
import assignment.librarymanager.data.UserBuilder;
...
public class UsersManager {
	...
	public UsersManager(Database database) {
	...
	private User parseResultSet(ResultSet resultSet) throws SQLException {
		...
		builder.setNotice(resultSet.getString("notice"));
	...
	public void insert(@NotNull User user, @NotNull Consumer<Integer> onSuccess, @NotNull Consumer<SQLException> onFailure) {
		String sql = "INSERT INTO users (username, name, pwd_hash, role, creation, email, notice) VALUES (?, ?, ?, ?, ?, ?, ?)";
		try (PreparedStatement statement = database.createPreparedStatement(sql)) {
			...
			statement.setString(7, user.getNotice());
	...
	public void update(@NotNull User user, @NotNull Consumer<Integer> onSuccess, @NotNull Consumer<SQLException> onFailure) {
		String sql = "UPDATE users SET name = ?, pwd_hash = ?, role = ?, email = ?, notice = ? WHERE username = ?";
		try (PreparedStatement statement = database.createPreparedStatement(sql)) {
			statement.setString(1, user.getName());
			statement.setString(2, user.getPasswordHash());
			statement.setString(3, user.getRole().name());
			statement.setString(4, user.getEmail());
			statement.setString(5, user.getNotice());
